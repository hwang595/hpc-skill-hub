...
python3 tools/proposal_evidence.py --owner <owner>
...
summary for the launch issue or owner handoff. The proposal evidence report
summarizes registry coverage, community assets, readiness checks, and review
pilot candidates for open-source review packets. The review candidate report
prints a local first-review queue for domain reviewer recruitment. They do not
create a repository, push commits, edit settings, open issues, or cut a release.
...
- `python3 tools/proposal_evidence.py --owner <owner> --run-check` produces a
  proposal evidence report for owner handoff or open-source review packets.
...
- `python3 tools/github_rulesets.py --repo <owner>/hpc-skill-hub` prints the
  ...
  workflow runs.
- `python3 tools/github_release.py v0.5.0 --repo <owner>/hpc-skill-hub` prints
...
- `python3 tools/github_homepage.py --repo <owner>/hpc-skill-hub` prints the
  command to inspect the Pages URL and set it as the repository homepage.
...
- The repository homepage points at the generated GitHub Pages site.
...
5. Link the deployed URL from the repository homepage:
