...
| Containers | Apptainer and Singularity build plans, runtime binds, image portability, and containerized job patterns. | `containers`, `container-build-for-hpc`, `apptainer-run-container` |
...
| MPI and performance | MPI launch, mpi4py, OpenMP placement, simple benchmarks, profiling evidence, and scaling sanity checks. | `gpu-mpi-performance`, `mpi-hello-and-benchmark`, `mpi4py-on-slurm`, `openmp-thread-affinity`, `performance-profile-basic` |
